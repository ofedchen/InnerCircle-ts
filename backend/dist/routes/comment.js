...
router.get("/:post_id", async (req, res) => {
    try {
        const result = await db.query("SELECT c.comment_id, c.comment_author, c.comment_text, c.comment_date, u.users_name FROM comment c JOIN users u ON c.comment_author=u.users_id WHERE post_id = $1 ORDER BY c.comment_date ASC", [req.params.post_id]);
        res.send(result.rows);
    }
    catch (err) {
...
router.post("/", async (req, res) => {
    ...
    try {
        const result = await db.query(`INSERT INTO comment (comment_author, comment_text, post_id) 
         VALUES($1, $2, $3) 
         RETURNING comment_id, comment_author, comment_text, comment_date, post_id`, [userId, commentText, postId]);

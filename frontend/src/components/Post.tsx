...
export default function Post({
	...
	return (
		...
		>
			...
			<div
				id="post-content"
				className={`rounded-md mt-[-50px] relative z-10 ${lockedPost}`}
			>
				{video && (
					<iframe
						data-cy="post-video"
				...
				{postImg && <img src={postImg} alt={title} data-cy="post-image" />}
			</div>
			<Link to={`/feed/${postId}`}>
				<h3
					data-cy="post-title"
			...
			<p
				className={`text-(--orange-white) py-2 ${lockedPost}`}
				data-cy="post-text"
			>
				{text}
			</p>
			{!blurred && (
				<Link to={`/feed/${postId}`}>
					<button
						data-cy="post-button"
						className="self-end mr-[-12px]  bg-(--purple-darker) text-(--orange-main) rounded-md px-4 py-2  hover:bg-(--purple-darker)"
					>
						{" "}
						TO POST{" "}
					</button>
